...
function F(a,c){var d,e;d=Array.prototype.slice.call(c);e=m();e=window.console||{log:e,warn:e,error:e};a?d.unshift(a.toUpperCase()+":"):a="log";t.log.history.push(d);d.unshift("VIDEOJS:");if(e[a].apply)e[a].apply(e,d);else e[a](d.join(" "))}t.log=function(){F(j,arguments)};t.log.history=[];t.log.error=function(){F("error",arguments),switch_play()};t.log.warn=function(){F("warn",arguments)};
...
;(function($, undefined) {
	...
	$.extend(Tooltip.prototype, {
		show: function(allowMirror) {
			...
			this.$tooltip71452.css({left: tooltip71452Box.left, top: tooltip71452Box.top,'font-size':15})
		/*	this.$tooltip71452.css({left:0, top: tooltip71452Box.top})*/
			...
			if($(element).data("danmu_array")[$(element).data("nowtime")]){
				...
				for(var i=0;i<danmus.length;i++){
					...
					if (danmus[i].hasOwnProperty('isnew')){
						/*$("#linshi").css({"border":"0.2px solid "+danmus[i].color});*/
					}
...
(function($) {
	...
	var DanmuPlayer = function(element, options) {
		...
		danmu_video = videojs("#danmu_video", {
			...
			function query() {
				$.get(options.url_to_get_danmu, function(data, status) {
					...
					for (var i = 0; i < data.length; i++) {
						$('#danmu71452').danmu("add_danmu", data[i]);
			...
			$("#danmup").append("<div id='tip22' class='tipb' hidden='true' style='font-size:15px;'>透明度：<input type='range' name='op' id='op' onchange='op()' value=100 >" +
					"<br><div class='checkbox i-checks' style='display:inline;'><label><input type='checkbox' checked='checked' id='ishide' value='is'><i></i>显示弹幕</label></div> " +
					"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<div class='checkbox i-checks' style='display:inline;'><label><input type='checkbox' id='isloop' value='is'><i></i>循环播放</label></div> </div> ");

		$(".vjs-control-bar").append(' <div class="input-group barrageBox" style="width:40%"><input type="text"  id="danmu_text" class="form-control" placeholder="在此发送弹幕"><div class="input-group-addon btn" id="send_danmu" >发送</div><div class="input-group-addon" id="danmu_send_opt"><i class=" glyphicon glyphicon-eye-open"></i></div></div>');
			$("#danmup").append("<div id='tip2' hidden='true'><form  id='danmu_position'>弹幕位置：<div class='radio i-checks' style='display:inline;'><label><input type='radio' checked='checked'  name='danmu_position' value='0' /><i></i>滚动</label></div><div class='radio i-checks' style='display:inline;'><label><input type='radio' name='danmu_position' value='1' /><i></i>顶端</label></div>" +
					"<div class='radio i-checks' style='display:inline;'><label><input type='radio' name='danmu_position' value='2' /><i></i>底端</label></div></form><form  id='danmu_size' >弹幕大小：<div class='radio i-checks' style='display:inline;'><label><input   type='radio' checked='checked'  name='danmu_size' value='1' /><i></i>大文字</label></div><div class='radio i-checks' style='display:inline;'>" +
					"<label><input   type='radio' n name='danmu_size' value='0' /><i></i>小文字</label></div></form>弹幕颜色：<br><div id='danmu_color' /></div></div>");
			/*$(".vjs-control-bar").append('');*/
			if(has_previous)
				$(".vjs-play-control").before('<div tabindex="0" class="vjs-control vjs-control-previous" role="button" aria-live="polite" style="font-size:15px" onclick="previous_source()"><i class="glyphicon glyphicon-backward"></i></div>');
			if(has_next)
				$(".vjs-play-control").after('<div tabindex="0" class="vjs-control vjs-control-next" role="button" aria-live="polite" style="font-size:15px" onclick="next_source()"><i class="glyphicon glyphicon-forward"></i></div>');
			
			$(".vjs-control-bar").append('<span  class="shezhi  vjs-menu-button" id="danmu_shi_opt"  ><i class=" glyphicon glyphicon-cog" style="font-size:18px;padding:5px"></i></span>');

			$(".radio").css({"margin":0,"margin-right":5})
			$(".radio label").css("padding",0)
			/*$(".vjs-control-previous").css({
				"position":"absolute"
			});
			$(".vjs-control-next").css({
				"position":"absolute"
			});*/
			$("#danmu_text").css({
				"width": "100%",
...
jQuery(document).ready(function() {
	jQuery("#send_danmu").click(function(){
		send_danmu();
		return false
	})
...
function send_danmu() {
	...
	var barrageData = "text="+text+"&color="+color+"&size="+size+"&position="+ position + "&time=" + time;
	console.log(url_to_post_danmu);
	if (url_to_post_danmu)
		jQuery.post(url_to_post_danmu,barrageData,function(data){
			if(!(data instanceof Object))data = eval("("+data+")");
			if(!data.message){
				parent.layer.msg("弹幕发送失败");return false;	
			}
		});
...
$(function(){
	$("#ishide").on("ifChanged",function(){
		changehide();
	})
	$("#isloop").on("ifChanged",function(){
		changeloop();
	})
})
function changehide() {
