        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function() {
                ...
                if($(this).is('li:has(ul)')) {
                    ...
                    text = $(this).text();
                    $(this).html('<span><span class="glyphicon"></span><a href="javascript: void(0);"></a> </span>');
                    $(this).find(' > span > span').addClass('glyphicon-leaf');
                    $(this).find(' > span > a').text(text);
                }
            });
            ...
            // addable
            if (options.addable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="create"><button class="btn btn-default btn-sm btn-success"><span class="glyphicon glyphicon-plus"></span>添加分类</button></div> ');
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    var createBlock = $(easyTree).find('.easy-tree-toolbar');
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        var item = $('<li><span><span class="glyphicon glyphicon-leaf"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        $(item).find(' > span > span').attr('title', options.i18n.collapseTip);
                        $(item).find(' > span > a').attr('title', options.i18n.selectTip);
                        if (selected.length <= 0) {
                        	$.post("category/0","name="+$(createInput).find('input').val(),function(data){
                        		if(data!=null){
                        			data = eval("("+data+")");
                        		}
                        		if(data.message){
                                    ...
                                    $(easyTree).find('.alert .alert-content').text(data.cause);*/
                        			parent.layer.msg('添加失败:'+data.cause, {shift : 2});
                        		}
                        	})
                        } else if (selected.length > 1) {
                            /*$(easyTree).prepend(warningAlert);
                            $(easyTree).find('.alert .alert-content').text(options.i18n.addMultiple);*/
                        	parent.layer.msg(options.i18n.addMultiple, {shift : 6});
                        } else {
                        	//添加分类
                        	$.post("category/"+$(selected).attr("data-id"),"name="+$(createInput).find('input').val(),function(data){
                        		if(data!=null){
                        			data = eval("("+data+")");
                        		}
                        		if(data.message){
                        			...
                        			if ($(selected).hasClass('parent_li')) {
                                        ...
                                        $(selected).addClass('parent_li').find(' > span > span').addClass('glyphicon-minus').removeClass('glyphicon-leaf');
            ...
            // editable
            if (options.editable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="edit"><button class="btn btn-default btn-sm btn-primary disabled"><span class="glyphicon glyphicon-edit"></span>编辑分类</button></div> ');
                $(easyTree).find('.easy-tree-toolbar .edit > button').attr('title', options.i18n.editTip).click(function () {
                    ...
                    if (selected.length <= 0) {
                       /* $(easyTree).prepend(warningAlert);
                    	...
                    	parent.layer.msg(options.i18n.editNull, {shift : 6});
                    }
                    else if (selected.length > 1) {
                       /* $(easyTree).prepend(warningAlert);
                        $(easyTree).find('.alert .alert-content').html(options.i18n.editMultiple);*/
                    	parent.layer.msg(options.i18n.editMultiple, {shift : 6});
                    }
                    else {
                        var value = $(selected).find(' > span > a').text().trim();
                        $(selected).find(' > span > a').hide();
                        $(selected).find(' > span').append('<input type="text" class="easy-tree-editor" style="color:#000">');
                        var editor = $(selected).find(' > span > input.easy-tree-editor');
                        $(editor).val(value);
                        $(editor).attr("data-source",value);
                        $(editor).focus();
                        $(editor).keydown(function (e) {
                            if (e.which == 13) {
                                if ($(editor).val() !== '') {
                                	//向数据库更新分类名字
                                	$.post("category/"+$(selected).attr("data-id"),"_method=PUT&name="+$(editor).val(),function(data){
                                		if(data!=null){
                                			data = eval("("+data+")");
                                			if(!data.message){
                                				 $(selected).find(' > span > a').text($(editor).attr("data-source"));
                                                 $(editor).remove();
                                                 $(selected).find(' > span > a').show();
                                                 /*$(easyTree).prepend(warningAlert);
                                                 $(easyTree).find('.alert .alert-content').html("更新分类失败！！"+data.cause);*/
                                                 parent.layer.msg('更新失败:'+data.cause, {
                                 					shift : 2
                                 				});
                                			}else{
            ...
            // deletable
            if (options.deletable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="remove"><button class="btn btn-default btn-sm btn-danger disabled"><span class="glyphicon glyphicon-remove"></span>删除分类</button></div> ');
                $(easyTree).find('.easy-tree-toolbar .remove > button').attr('title', options.i18n.deleteTip).click(function () {
                    ...
                    if (selected.length <= 0) {
                       /* $(easyTree).prepend(warningAlert);
                    	...
                    	parent.layer.msg(options.i18n.deleteNull, {shift : 6});
                    } else {
                    	...
                    	parent.layer.confirm('确认要执行删除操作吗？', {
                    	    btn: ['马上给我删除','让我再想想'], //按钮
                    	    shade: false //不显示遮罩
                    	}, function(){
                    		//删除分类
                        	$.post("category/"+$(selected).attr("data-id"),"_method=DELETE",function(data){
                        		if(data!=null){
                        			data = eval("("+data+")");
                        			if(!data.message){
                        				/*$(easyTree).prepend(warningAlert);
                        				$(easyTree).find('.alert .alert-content').html("删除失败！！"+data.cause);*/
                                        ...
                                        if($(selected).parent('ul').find(' > li').length <= 1) {
                                            $(selected).parents('li').removeClass('parent_li').find(' > span > span').removeClass('glyphicon-minus').addClass('glyphicon-leaf');
                    	    ...
                    	    parent.layer.msg('已取消删除操作', {shift: 6});
                    	});
